...
::: tip Generated on 2026-04-23 17:59:39, from kubo 0.40.1
...
OPTIONS
  ...
  --enable-pubsub-experiment      bool   - DEPRECATED CLI flag. Use
                                           Pubsub.Enabled config instead.
  --enable-namesys-pubsub         bool   - DEPRECATED CLI flag. Use
                                           Ipns.UsePubsub config instead.
...
OPTIONS
  ...
  -p, --progress  bool - Show progress on stderr. Auto-detected if stderr is a
                         terminal.
...
EXPERIMENTAL SUBCOMMANDS
  ipfs diag datastore - Low-level datastore inspection for debugging and
                        testing.
...
EXPERIMENTAL SUBCOMMANDS
  ipfs diag datastore count <prefix> - Count entries matching a datastore
                                       prefix.
  ipfs diag datastore get <key>      - Read a raw key from the datastore.


```

## ipfs diag datastore count
...
SYNOPSIS
  ipfs diag datastore count [--] <prefix>

ARGUMENTS

  <prefix> - Datastore key prefix (e.g., /pubsub/seqno/)
...
SYNOPSIS
  ipfs diag datastore get [--hex] [--] <key>

ARGUMENTS

  <key> - Datastore key to read (e.g., /pubsub/seqno/<peerid>)

OPTIONS

  --hex  bool - Output hex dump instead of raw bytes.
...
EXPERIMENTAL SUBCOMMANDS
  ...
  ipfs files chroot [<cid>]      - Change the MFS root CID.
...
ARGUMENTS
  ...
  [<cid>] - New root CID (defaults to empty directory if not specified).

OPTIONS

  --confirm  bool - Confirm this potentially destructive operation.
...
SUBCOMMANDS
  ...
  ipfs key ls                      - List all local keypairs.
...
DEPRECATED SUBCOMMANDS
  ipfs key list - Deprecated: use 'ipfs key ls' instead.
...
SYNOPSIS
  ipfs key ls [-l] [--ipns-base=<ipns-base>]
...
OPTIONS
  ...
  -k, --key        string - The name of the key to use for verifying.
...
OPTIONS
  ...
  -s, --stream    bool - Enable experimental streaming of directory entries as
                         they are traversed.
  -l, --long      bool - Use a long listing format, showing file mode and
                         modification time.
...
EXPERIMENTAL SUBCOMMANDS
  ipfs name get <name>          - Retrieve a signed IPNS record.
  ipfs name inspect <record>    - Inspects an IPNS Record
  ipfs name pubsub              - IPNS pubsub management
  ipfs name put <name> <record> - Store a pre-signed IPNS record in the routing
                                  system.
...
SYNOPSIS
  ipfs name put [--force | -f] [--allow-offline] [--allow-delegated]
                [--quiet | -q] [--] <name> <record>

ARGUMENTS

  <name>   - The IPNS name to store the record for (e.g., k51... or
             /ipns/k51...).
  <record> - Path to file containing the signed IPNS record.

OPTIONS

  -f, --force        bool - Skip validation (signature, sequence, size).
  --allow-offline    bool - Store locally without broadcasting to the network.
  --allow-delegated  bool - Publish via HTTP delegated publishers only (no DHT).
  -q, --quiet        bool - Write no output.
...
OPTIONS
  ...
  --allow-big-block  bool - Disable block size check and allow creation of
                            blocks bigger than 2MiB. WARNING: such blocks won't
...
SYNOPSIS
  ipfs p2p forward [--allow-custom-protocol] [--foreground | -f] [--] <protocol>
                   <listen-address> <target-address>
...
OPTIONS
  ...
  -f, --foreground         bool - Run in foreground; forwarder is removed when
                                  command exits.
...
DESCRIPTION
  ...
  ARGUMENTS:
    ...
    <protocol>        Protocol name (must start with '/x/')
...
SYNOPSIS
  ipfs p2p listen [--allow-custom-protocol] [--report-peer-id | -r]
                  [--foreground | -f] [--] <protocol> <target-address>
...
OPTIONS
  ...
  -f, --foreground         bool - Run in foreground; listener is removed when
                                  command exits.
...
EXPERIMENTAL SUBCOMMANDS
  ...
  ipfs pubsub reset              - Reset pubsub validator state.
...
SYNOPSIS
  ipfs pubsub reset [--peer=<peer> | -p]

OPTIONS

  -p, --peer  string - Only reset state for this peer ID.
...
SYNOPSIS
  ipfs repo verify [--drop] [--heal] [--heal-timeout=<heal-timeout>]

OPTIONS

  --drop          bool   - Remove corrupt blocks from datastore (destructive
                           operation).
  --heal          bool   - Remove corrupt blocks and re-fetch from network
                           (destructive operation, implies --drop).
  --heal-timeout  string - Maximum time to wait for each block heal (e.g.,
                           "30s"). Only applies with --heal. Default: 30s.
...
SYNOPSIS
  ipfs stats provide [--lan] [--all | -a] [--compact] [--connectivity]
                     [--network] [--schedule] [--timings] [--workers]
                     [--operations] [--queues]
...
SYNOPSIS
  ipfs stats reprovide [--lan] [--all | -a] [--compact] [--connectivity]
                       [--network] [--schedule] [--timings] [--workers]
                       [--operations] [--queues]
...
SUBCOMMANDS
  ipfs swarm addrs autonat - Show address reachability as determined by AutoNAT
                             V2.
