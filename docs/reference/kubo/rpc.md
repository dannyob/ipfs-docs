...
::: tip Generated on 2026-04-23, from kubo v0.40.1
...
`curl -X POST -F file=@myfile "http://127.0.0.1:5001/api/v0/dag/import?pin-roots=true&silent=<value>&stats=<value>&fast-provide-root=<value>&fast-provide-wait=<value>&allow-big-block=false"`
...
- `progress` [bool]: Show progress on stderr. Auto-detected if stderr is a terminal. Required: no.
...
`curl -X POST "http://127.0.0.1:5001/api/v0/key/ls?l=<value>&ipns-base=base36"`
...
Change or get the logging level.

### Arguments

- `arg` [string]: The subsystem logging identifier. Use &#39;all&#39; or &#39;*&#39; to get or set the log level of all subsystems including the default. If not specified, only show the default log level. Required: no.
- `arg` [string]: The log level, with &#39;debug&#39; as the most verbose and &#39;fatal&#39; the least verbose. Use &#39;default&#39; to set to the current default level. One of: debug, info, warn, error, dpanic, panic, fatal, default Required: no.
...
- `long` [bool]: Use a long listing format, showing file mode and modification time. Required: no.
...
`curl -X POST "http://127.0.0.1:5001/api/v0/ls?arg=<ipfs-path>&headers=<value>&resolve-type=true&size=true&stream=<value>&long=<value>"`
...
- `allow-delegated` [bool]: Allow publishing without DHT connectivity - uses local datastore and HTTP delegated publishers only. Required: no.
- `sequence` [uint64]: Set a custom sequence number for the IPNS record (must be higher than current). Required: no.
- `ipns-base` [string]: Encoding used for keys: Can either be a multibase encoded CID or a base58btc encoded multihash. Takes {b58mh|base36|k|base32|b...}. Default: `base36`. Required: no.
...
- `drop` [bool]: Remove corrupt blocks from datastore (destructive operation). Required: no.
- `heal` [bool]: Remove corrupt blocks and re-fetch from network (destructive operation, implies --drop). Required: no.
- `heal-timeout` [string]: Maximum time to wait for each block heal (e.g., &#34;30s&#34;). Only applies with --heal. Default: `30s`. Required: no.
...
`curl -X POST "http://127.0.0.1:5001/api/v0/repo/verify?drop=<value>&heal=<value>&heal-timeout=30s"`
...
## /api/v0/swarm/addrs/autonat

Show address reachability as determined by AutoNAT V2.
...
`curl -X POST "http://127.0.0.1:5001/api/v0/swarm/addrs/autonat"`
...
- `key` [string]: The name of the key to use for verifying. Required: no.
...
Retrieve a signed IPNS record.

### Arguments

- `arg` [string]: The IPNS name to look up. Required: **yes**.
...
`curl -X POST "http://127.0.0.1:5001/api/v0/name/get?arg=<name>"`
...
Store a pre-signed IPNS record in the routing system.

### Arguments

- `arg` [string]: The IPNS name to store the record for (e.g., k51... or /ipns/k51...). Required: **yes**.

- `force` [bool]: Skip validation (signature, sequence, size). Required: no.
- `allow-offline` [bool]: Store locally without broadcasting to the network. Required: no.
- `allow-delegated` [bool]: Publish via HTTP delegated publishers only (no DHT). Required: no.
- `quiet` [bool]: Write no output. Required: no.
...
`curl -X POST -F file=@myfile "http://127.0.0.1:5001/api/v0/name/put?arg=<name>&force=<value>&allow-offline=<value>&allow-delegated=<value>&quiet=<value>"`
...
- `foreground` [bool]: Run in foreground; forwarder is removed when command exits. Required: no.
...
`curl -X POST "http://127.0.0.1:5001/api/v0/p2p/forward?arg=<protocol>&arg=<listen-address>&arg=<target-address>&allow-custom-protocol=<value>&foreground=<value>"`
...
- `foreground` [bool]: Run in foreground; listener is removed when command exits. Required: no.
...
`curl -X POST "http://127.0.0.1:5001/api/v0/p2p/listen?arg=<protocol>&arg=<target-address>&allow-custom-protocol=<value>&report-peer-id=<value>&foreground=<value>"`
...
Clear all CIDs from the provide queue.

### Arguments

- `quiet` [bool]: Do not write output. Required: no.
...
`curl -X POST "http://127.0.0.1:5001/api/v0/provide/clear?quiet=<value>"`
...
Show statistics about the provider system

### Arguments

- `lan` [bool]: Show stats for LAN DHT only (for Sweep+Dual DHT only). Required: no.
- `all` [bool]: Display all provide sweep stats. Required: no.
- `compact` [bool]: Display stats in 2-column layout (requires --all). Required: no.
- `connectivity` [bool]: Display DHT connectivity status. Required: no.
- `network` [bool]: Display network stats (peers, reachability, region size). Required: no.
- `schedule` [bool]: Display reprovide schedule (CIDs/regions scheduled, next reprovide time). Required: no.
- `timings` [bool]: Display timing information (uptime, cycle start, reprovide interval). Required: no.
- `workers` [bool]: Display worker pool stats (active/available/queued workers). Required: no.
- `operations` [bool]: Display operation stats (ongoing/past provides, rates, errors). Required: no.
- `queues` [bool]: Display provide and reprovide queue sizes. Required: no.
...
```json
{
  ...
  "Sweep": {
    ...
    "network": {
      "avg_holders": "<float64>",
      "avg_region_size": "<float64>",
      "complete_keyspace_coverage": "<bool>",
      "peers": "<int>",
      "reachable": "<int>",
    ...
    "operations": {
      "ongoing": {
        ...
        "key_reprovides": "<int>",
        "region_provides": "<int>",
        "region_reprovides": "<int>"
      },
      "past": {
        "avg_keys_per_reprovide": "<float64>",
        "keys_failed": "<int64>",
        "keys_provided": "<int64>",
        "keys_provided_per_minute": "<float64>",
        "keys_reprovided_per_minute": "<float64>",
        "records_provided": "<int64>",
        "regions_reprovided_last_cycle": "<int64>",
        "reprovide_duration": "<duration-ns>"
      }
    },
    "queues": {
      "pending_key_provides": "<int64>",
      "pending_region_provides": "<int64>",
      "pending_region_reprovides": "<int64>"
    },
    "schedule": {
      "avg_prefix_length": "<float64>",
      "keys": "<int64>",
      "next_reprovide_at": "<timestamp>",
      "next_reprovide_prefix": "<string>",
      "regions": "<int64>"
    },
    "timing": {
      "current_time_offset": "<duration-ns>",
      "cycle_start": "<timestamp>",
      "max_reprovide_delay": "<duration-ns>",
      "reprovides_interval": "<duration-ns>",
      "uptime": "<duration-ns>"
    },
    "workers": {
      "active": "<int>",
      ...
      "active_periodic": "<int>",
      "dedicated_burst": "<int>",
      "dedicated_periodic": "<int>",
      "max": "<int>",
      "max_provide_conns_per_worker": "<int>",
      "queued_burst": "<int>",
      "queued_periodic": "<int>"
    }
  }
...
`curl -X POST "http://127.0.0.1:5001/api/v0/provide/stat?lan=<value>&all=<value>&compact=<value>&connectivity=<value>&network=<value>&schedule=<value>&timings=<value>&workers=<value>&operations=<value>&queues=<value>"`
...
Reset pubsub validator state.

### Arguments

- `peer` [string]: Only reset state for this peer ID. Required: no.
...
`curl -X POST "http://127.0.0.1:5001/api/v0/pubsub/reset?peer=<value>"`
...
Deprecated: use &#39;ipfs key ls&#39; instead.
...
Deprecated command, use &#39;ipfs provide stat&#39; instead.
...
`curl -X POST "http://127.0.0.1:5001/api/v0/stats/provide?lan=<value>&all=<value>&compact=<value>&connectivity=<value>&network=<value>&schedule=<value>&timings=<value>&workers=<value>&operations=<value>&queues=<value>"`
...
`curl -X POST "http://127.0.0.1:5001/api/v0/stats/reprovide?lan=<value>&all=<value>&compact=<value>&connectivity=<value>&network=<value>&schedule=<value>&timings=<value>&workers=<value>&operations=<value>&queues=<value>"`
